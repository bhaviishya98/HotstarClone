...
const Detail = (props) => {
  ...
  const details = () => {
    fetch(
      `https://api.themoviedb.org/3/movie/${id}?api_key=9c9507db04f409d6558ccc85828c568d`
    )
      .then((res) => res.json())
      .then((json) => {
        console.log(json); // Check the response structure here
        setDetailData(json); // Just set the response directly
      })
